# ============================================
# Complete Redeployment Script with Wait
# Waits for SSH rate limit to clear
# ============================================
...
# Check if package already exists (from previous run)
if (Test-Path "tqd-deploy.tar.gz") {
    Write-Host "Found existing deployment package" -ForegroundColor Yellow
    $rebuild = Read-Host "Rebuild application? (y/n)"
...
# Wait for SSH rate limit to clear
Write-Host "Waiting $WAIT_SECONDS seconds for SSH rate limit to clear..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "The server may still be rate-limiting connections." -ForegroundColor Yellow
    Write-Host "Please wait longer and try again, or contact your hosting provider." -ForegroundColor Yellow
    exit 1
}
...
Write-Host "[5/6] Installing on server (this may take 2-3 minutes)..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Mobile fixes deployed:" -ForegroundColor Green
Write-Host "  - Custom cursor hidden on touch devices" -ForegroundColor White
Write-Host "  - Normal tap/touch interactions work" -ForegroundColor White
Write-Host "  - Gyroscope tilt improved" -ForegroundColor White
Write-Host ""
